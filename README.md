...
### Checked against the approving reviewer

In CI the name is not taken on trust. Every newly added `decisions/` entry must
name the GitHub login of someone who approved **that exact head**, and the check
compares it against the reviews:

```
FAIL: approval: recorded approver did not approve this head —
      decisions/2026-09-16-x.md names 'someone-else'; approving reviews
      of this head came from: jirbis
```

An approval is of a commit, not of a pull request, so an entry added after the
reviewer looked does not inherit their approval. Review submission, edit and
dismissal retrigger the check. Only entries the change adds are examined; the
record stays immutable. It fails closed: an approver list that was never written
means the query failed, which is never a pass.

It runs in a job that installs nothing and runs no project target. The gate's own
report deliberately does not perform it — that job runs your `make test`, and a
test recipe could write the approver list it is judged by.

**What this is, and is not.** It surfaces a mismatch between the recorded name
and the accounts that approved. It does not make approval mandatory, and it is
not a defence against a hostile author: the workflow and the verifier both come
from the branch under review.

**Requiring approval is GitHub's job**, not the gate's. On the default branch,
set a branch protection rule or ruleset with: a pull request required before
merging; required approving reviews; **dismissal of stale approvals on new
commits**; the `approval` check required; and no bypass for administrators.
Without those the gate is advisory, and a push straight to the default branch
has no reviews to check against at all.
